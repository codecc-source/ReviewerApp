...
class NotesReviewer:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Loraine Notes Reviewer")
        self.notes = {}
        self.load_notes()
        ...
        self.browse_image_button = tk.Button(self.menu_frame, text="Browse Image (pra sa visual questions)", command=self.browse_image, font=button_font, bg='#EA3B52', fg='white', width=50)
        ...
        self.clear_button = tk.Button(self.menu_frame, text="Clear Notes (click lng ni if tapos kna tuon sa isa ka subject)", command=self.clear_notes, font=button_font, width=50, bg='#666362', fg='white')
        self.clear_button.pack()

        self.undo_button = tk.Button(self.menu_frame, text="Undo (if ma typo)", command=self.undo_last_note, font=button_font, width=50, bg='#666362', fg='white')
        ...
        self.export_button = tk.Button(self.menu_frame, text="Export to Word (png print/notes)", command=self.export_to_word, font=button_font, width=50, bg='#666362', fg='white')
        ...
        self.json_filename_label = tk.Label(self.menu_frame, text="Note File Name (Make sure na may (dot).json ang end. \nEx: Subject1.json):", font=label_font, width=50, bg='#3B3131', fg='white')
    ...
    def load_image(self, file_path):
        try:
            ...
            self.image_preview_window.title("Notes Image Review (Ma auto close lng ni pag ma click ka Add Notes)")
    ...
    def review_notes(self):
        ...
        for question in questions:
            while True:
                if question.startswith("[Image]"):
                    ...
                    photo = ImageTk.PhotoImage(image)
                    image_label = tk.Label(self.menu_frame, image=photo)
        ...
        if correct_count > 0:
            messagebox.showinfo("Score mo Loraine", review_message + "\n\n" + correct_answers_message)
        else:
            messagebox.showinfo("Sad Retry Loraine", review_message + "\n\n" + correct_answers_message)
    ...
    def edit_note(self):
        ...
        browse_image_button = tk.Button(edit_window, text="Browse New Image", command=browse_new_image, font=label_font)
    ...
    def save_notes_to_json(self):
        ...
        if not json_filename:
            messagebox.showerror("Error", "Please enter a valid JSON file name.")
            return
